## Overview

Linux-API is a small REST API project written in Python (FastAPI) that collects and exposes system and application metrics. It uses PostgreSQL for storage and includes Docker Compose definitions to run required services.

## Documentation

- Manual setup (step-by-step): [Manual Setup](https://github.com/Ivole32/Linux-API/wiki/Manual-setup-for-Linux%E2%80%90API)
- Automated setup: [Automated setup](https://github.com/Ivole32/Linux-API/wiki/Automated-setup-for-Linux%E2%80%90API) (Not recommendet)
- Project overview: [README](https://github.com/Ivole32/Linux-API/blob/main/README.md)

## Support

- Issues: https://github.com/Ivole32/Linux-API/issues
- Support the project: https://ko-fi.com/ivole32

---

Made with ❤️ by Ivole32 — https://github.com/Ivole32