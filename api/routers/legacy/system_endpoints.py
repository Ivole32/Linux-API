...
async def get_uptime(request: Request, user_data = get_user_role("user")):
...
async def get_processes(request: Request, user_data = get_user_role("user")):
    processes = list_processes()
    return processes

@router.get("/system-infos", description="This endpoint returns the system information of the server.")
@limiter.limit("10/minute")
async def system_infos(request: Request, user_data = get_user_role("user")):
...
@limiter.limit("5/minute")
async def avg_load(request: Request, decimal_places: int = 2, last_load_length: int = 3, user_data = get_user_role("user")):
